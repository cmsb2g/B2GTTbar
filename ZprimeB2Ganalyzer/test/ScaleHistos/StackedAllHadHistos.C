...
void draw(string variable, string x_axis_name, string cut, string outlabel, int nBins, float min, float max, double xInfo, double yInfoTop, int extra_cut_label){
  ...
  //zpn1000tree->Draw( Form("%s>>zpn1H", variable.c_str(), nBins, min, max), Form("%s", cut.c_str()), "goff");
  ...
  dataH->SetMinimum(0.001);
  dataH->GetXaxis()->SetTitle(x_axis_name.c_str());
  dataH->GetYaxis()->SetTitle("Fraction of Events / Bin");
