...
void callHistos();
void StackedHistoMachineAK8_MSDjet_signal_tau32cut07();
...
// For./Input/AK8minmass-TTJet_WJetLNu-TTJet-ST.txt - move x below to 0.131 was 0.1 by default 
  tlx->DrawLatex(0.141, 0.868, "#bf{CMS Top Tagger}"); // same for AK4 and AK8 
  tlx->DrawLatex(0.141, 0.826, "#bf{Madgraph}"); // same for AK4 and AK8                   
  tlx->DrawLatex(0.141, 0.789, "#bf{AK R= 0.8}"); // same for AK4 and AK8
  tlx->DrawLatex(0.141, 0.755, "#bf{|#eta| < 2.4 }"); // > 400 for AK8 and > 30 for AK4
  tlx->DrawLatex(0.141, 0.712, "#bf{P_{T} > 400 GeV}");
  tlx->DrawLatex(0.141, 0.676, "#bf{50 ns}"); 
  tlx->DrawLatex(0.141, 0.636, "#bf{#tau_{3}/#tau_{2} < 0.7}"); 
  canvas->Update();
...
//  legend->AddEntry(histo5,"Z + Jets", "f");
 ...
 // Name the stacked histogram
  canvas->Print("./Stacked/nMinus1plots-tau32cut07-"+CalledHisto+"-"+day+"_"+month+"_"+year+".png");
