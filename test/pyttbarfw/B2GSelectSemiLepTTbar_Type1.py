...
class B2GSelectSemiLepTTbar_Type1( ) :
    ...
    def select( self ) :
        ...
        if not (self.ak8Jet.Perp() > 400. and abs(self.ak8Jet.Eta()) < 2.4 and self.ak8Jet.DeltaR( self.lepsel.leptonP4) > 1.0 ) : return passed
        passed[1] = True

        if not ( self.ak4JetBdisc > self.bdiscmin ) : return passed
        passed[2] = True
        
        if not ( 110. < self.ak8SDJet.M() < 250. ) : return passed
        passed[3] = True

        if not ( self.tau32 < self.tau32Cut ) : return passed
        passed[4] = True
